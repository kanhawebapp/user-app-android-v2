...
export const requestMediaPermissions = async (): Promise<{
  ...
  // Audio-only calls only require microphone permission
  const microphone = await requestMicrophonePermission();
  // Camera not needed, treat as granted
  return {camera: true, microphone};
};
...
export const checkMediaPermissions = async (): Promise<{
  ...
  if (Platform.OS === 'android') {
    ...
    // Camera not required for audio-only calls
    return {camera: true, microphone};
  }
