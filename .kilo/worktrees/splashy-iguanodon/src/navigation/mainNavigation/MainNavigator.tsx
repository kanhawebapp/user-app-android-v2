...
import SelectAstrologerScreen, {Astrologer} from '../../screens/main/healings/SelectAstrologerScreen';

// Hooks for healing flow
...
const MainNavigator: React.FC<MainNavigatorProps> = ({
  ...
  const [healingBookingData, setHealingBookingData] = useState<any>(null);

  // Healing hooks (for booking flow)
  const {submitBooking} = useCreateServiceBooking();
  const {createOrder} = useCreateHealingOrder();
  ...
  const handleBookingSubmit = useCallback(async (formData: BookingFormData) => {
    try {
      ...
      setHealingBookingData(response);
  ...
  const handlePayment = async (astrologer: Astrologer) => {
    try {
      const order = await createOrder(healingBookingData?.id);
      ...
      // Reset healing flow after successful payment
      setHealingScreen(null);
      setHealingSelectedService(null);
      setHealingBookingData(null);
    } catch (error: any) {
