 ...
 */
export const useRequireAuth = (props?: UseRequireAuthProps): UseRequireAuthReturn => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [modalMessage, setModalMessage] = useState(props?.message || 'Please login to perform this action');
   ...
   */
  const checkAuth = useCallback((customMessage?: string, onSuccess?: () => void): boolean => {
