...
export const useSignup = ({
  ...
  const signup = useCallback(async () => {
    ...
    try {
      setIsSigningUp(true);
      setSignupError(null);
      loggingService.info('[Signup] Starting signup process:', { name, phone, email });
      ...
      loggingService.info('[Signup] Signup successful:', { phone, user: mockUser.name });
      onSignupSuccess?.();
    } catch (error) {
  ...
  }, [name, email, phone, password, onSignupSuccess, onSignupError, setIsLoggedIn, authLogin]);
  ...
  const signupWithGoogle = useCallback(async () => {
    try {
      ...
      loggingService.info('[Signup] Google signup successful', { user: mockUser.name });
      onSignupSuccess?.();
    } catch (error) {
      const err = error instanceof Error ? error : new Error('Google signup failed');
  ...
  const signupWithFacebook = useCallback(async () => {
    try {
      ...
      // Create mock user and login to auth store
      const mockUser = createMockUser('Facebook User', '', 'facebook@example.com');
      await authLogin(mockUser, 'mock_facebook_access_token', 'mock_facebook_refresh_token');
      
      // Set logged in state
      setIsLoggedIn(true);
      
      loggingService.info('[Signup] Facebook signup successful', { user: mockUser.name });
      onSignupSuccess?.();
    } catch (error) {
      const err = error instanceof Error ? error : new Error('Facebook signup failed');
