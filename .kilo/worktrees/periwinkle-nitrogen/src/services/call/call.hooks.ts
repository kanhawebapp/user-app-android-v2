...
export const useCall = () => {
  ...
  useEffect(() => {
    // Start countdown ONCE when transitioning to connected
    if (status === 'connected' && !countdownStartedRef.current) {
    ...
    // Clear interval when status leaves 'connected'
    if (status !== 'connected') {
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
        intervalRef.current = null;
        console.log('[Timer] Interval cleared (status not connected)');
      }
      countdownStartedRef.current = false;
    }

    // Cleanup on unmount only - do NOT reset countdownStartedRef to prevent re-triggering
    return () => {
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
        intervalRef.current = null;
