...
import { webRTCService } from '../call';
...
export const useChatSocket = (socket: Socket | null) => {
  ...
  // console.log("--=-=-=-=-==--==-=paoo", userPayload)
  //   const roomId = useChatStore(state => state.roomId);
  // const setQueueData = useChatStore(state => state.setQueueData);
  ...
  const registerListeners = useCallback(() => {
    ...
    socket.on(SOCKET_EVENTS.CHAT_ACCEPTED, data => {
      ...
      console.log('[ChatSocket] Chat accepted:', { incoming: incomingRoomId, current: currentRoomId });
