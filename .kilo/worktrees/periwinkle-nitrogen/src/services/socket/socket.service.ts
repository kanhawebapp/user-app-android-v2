...
class SocketService {
  ...
  private handleChatAccepted = (
    ...
    if (terminalOrActiveStates.has(storeState.chatStatus)) {
      console.warn(
        ...
        { incoming: incomingRoomId, current: currentRoomId, status: storeState.chatStatus },
      );
      return;
    }
    ...
    if (this.chatAcceptedHandled) {
      console.log('[SocketService] Chat already accepted for this session, ignoring duplicate');
      return;
    }
  ...
  private registerGlobalListeners(
    ...
    (socket as any).on(SOCKET_EVENTS.QUEUE_UPDATE, (data: RawQueueData) => {
      ...
      const storeState = useChatStore.getState();
      if (terminalOrActiveStates.has(storeState.chatStatus)) return;
