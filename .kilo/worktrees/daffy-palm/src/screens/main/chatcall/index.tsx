...
const ChatCallScreen: React.FC<ChatCallScreenProps> = ({
  ...
  // Handle call button press — respects queue flow via chat.service
  const handleCallPress = useCallback(
    (astrologer: Astrologer) => {
      handleRestrictedAction(
        CHAT_CALL_LABELS.LOGIN_REQUIRED_CALL(astrologer.name),
        async () => {
          // Use sendChatRequest which attaches a queue_position listener
          // in the same synchronous block as the call_request emit.
          // If astrologer is busy: returns after queue clears → shows queue
          // bubble first, then navigation follows. If astrologer is free:
          // resolves immediately → direct call flow unchanged.
          try {
            const result = await sendChatRequest({
              ...
              // Reuse existing user data for the intake call
              name: user?.name || 'User',
              gender: (user?.gender as 'male' | 'female' | 'other') || 'male',
              dateOfBirth: user?.dateOfBirth || '',
              placeOfBirth: user?.birthPlace || user?.placeOfBirth || '',
              birthTime: user?.birthTime || '',
              occupation: user?.occupation || '',
              consultationType: 'call',
            });

            console.log('[ChatCallScreen] call_request result:', result);

            // QueueBubble is shown by the global socket listener when
            // position >= 1. We navigate here only when the queue has
            // cleared or when no queue exists.
            if (result.isQueued) {
              console.log(
                '[ChatCallScreen] CALLSCREEN_NAVIGATION_SKIPPED: wait for queue',
              );
              // QueueBubble is visible; stay on chatcall screen.
              setShowChatRequestModal(false);
              setSelectedAstrologer(null);
            } else {
            ...
            console.error('[ChatCallScreen] call_request failed:', err);
          }
        },
      );
    },
    [handleRestrictedAction, navigation, user],
  );
