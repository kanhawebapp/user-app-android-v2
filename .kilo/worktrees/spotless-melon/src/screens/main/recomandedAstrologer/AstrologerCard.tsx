...
const AstrologerCard: React.FC<AstrologerCardProps> = ({
  ...
  */
  ...
  const getPricing = (type: string) => {
    const pricingItem = item?.pricing?.find(
      (p: any) => p.type?.toUpperCase() === type,
    );

    return pricingItem;
  };

  // const chatPrice = activeOffer ? activeOffer.price : getPricing('CHAT')?.offerPrice || getPricing('CHAT')?.price || 0;

  // const callPrice = activeOffer ? activeOffer.price : getPricing('CALL')?.offerPrice || getPricing('CALL')?.price || 0;
