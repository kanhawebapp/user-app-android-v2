...
import { HEADER_IMAGE_OPTIONS, DEFAULT_HEADER_IMAGE } from '../helpers/headerImageHelpers';
import { playAarti, stopAarti, playEffect, preloadSounds } from '../utils/SoundService';
...
export const useDailyPuja = (): UseDailyPujaReturn => {
  ...
  const [selectedGodTem, setSelectedGodTem] = useState<ExtendedGod>(getDefaultGod());
  ...
  const [activeSpecialItem, setActiveSpecialItem] = useState<string | null>(null);
  const [isAartiPlaying, setIsAartiPlaying] = useState(false);
  
  // Thali and items state
  const [showThali, setShowThali] = useState(true);
  const [selectedItems, setSelectedItems] = useState<Record<string, PujaItemOption>>({});
  ...
  const [selectedHeaderImage, setSelectedHeaderImage] = useState<HeaderImageOption>(DEFAULT_HEADER_IMAGE);
  ...
  useEffect(() => {
    ...
    preloadSounds().then(() => {
      ...
      console.error('[useDailyPuja] Failed to preload sounds:', err);
    });
  }, []);
  ...
  const handleSelectItemOption = useCallback((option: PujaItemOption) => {
    if (selectedItemId) {
  ...
  const handleSpecialItemPress = useCallback((itemId: string, soundFile: string) => {
