...
import type { ProfileFieldKey, ProfileField, PendingProfileRequest } from '../../types/profile.types';
...
const ProfileCompletionModal: React.FC<ProfileCompletionModalProps> = ({
  ...
  const [errors, setErrors] = useState<Record<ProfileFieldKey, string>>({} as any);

  // Get fields to display based on current request
  const fieldsToDisplay = useMemo((): ProfileField[] => {
    if (!currentRequest) return [];
    
    const allFields = [...currentRequest.requiredFields, ...currentRequest.optionalFields];
  ...
  // Handle input change
  const handleInputChange = useCallback((field: ProfileFieldKey, value: string) => {
    ...
    fieldsToDisplay.forEach(field => {
      ...
      if (value && field.validation) {
        if (field.validation.minLength && value.length < field.validation.minLength) {
          newErrors[field.key] = field.validation.errorMessage || `Minimum ${field.validation.minLength} characters required`;
          isValid = false;
        }
        if (field.validation.maxLength && value.length > field.validation.maxLength) {
          newErrors[field.key] = field.validation.errorMessage || `Maximum ${field.validation.maxLength} characters allowed`;
          isValid = false;
        }
        if (field.validation.pattern && !field.validation.pattern.test(value)) {
          newErrors[field.key] = field.validation.errorMessage || 'Invalid format';
          isValid = false;
        }
      }
    });
  ...
  }, [validateForm, formData, updateProfileFields, completeProfileRequest, onComplete]);
  ...
  const renderField = useCallback((field: ProfileField) => {
    const value = formData[field.key];
          ...
          <View style={styles.selectContainer}>
            {field.options.map((option) => (
              ...
              >
                <Text
                  variant="body"
                  color={value === option.value ? colors.primary.main : colors.text.primary}
