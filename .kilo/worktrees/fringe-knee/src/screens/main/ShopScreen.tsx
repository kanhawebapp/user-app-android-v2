...
const products = [
  { id: 1, name: 'Blue Sapphire', category: 'Gemstones', price: 2500, rating: 4.5, image: null },
  { id: 2, name: 'Ruby Ring', category: 'Gemstones', price: 3500, rating: 4.8, image: null },
  { id: 3, name: 'Vastu Yantra', category: 'Yantras', price: 800, rating: 4.3, image: null },
  { id: 4, name: 'Hanuman Yantra', category: 'Yantras', price: 600, rating: 4.6, image: null },
  { id: 5, name: 'Vedic Astrology Book', category: 'Books', price: 450, rating: 4.7, image: null },
  { id: 6, name: 'Puja Kit', category: 'Kits', price: 1200, rating: 4.4, image: null },
];
...
const ShopScreen: React.FC<ShopScreenProps> = ({ 
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesCategory = selectedCategory === 1 || product.category === categories[selectedCategory - 1].name;
  ...
  const renderProductCard = (product: typeof products[0]) => (
    <Card key={product.id} style={styles.productCard}>
      <View style={[styles.productImage, { backgroundColor: colors.background.secondary }]}>
        <Icon name="shopping-bag" size={32} color={colors.icon.tertiary} library="Feather" />
      </View>
      <View style={styles.productInfo}>
        <Text variant="label" weight="semibold" numberOfLines={1}>{product.name}</Text>
        <Text variant="captionSmall" style={{ color: colors.text.secondary }}>{product.category}</Text>
        <View style={styles.ratingRow}>
          <Icon name="star" size={12} color={colors.common.yellow[500]} library="MaterialIcons" />
          <Text variant="captionSmall" style={{ marginLeft: 4 }}>{product.rating}</Text>
        </View>
        <View style={styles.priceRow}>
          <Text variant="subtitle" weight="bold" style={{ color: colors.primary.main }}>₹{product.price}</Text>
          <TouchableOpacity
            style={[styles.addButton, { backgroundColor: colors.primary.main }]}
            onPress={handleAddToCart}
          >
<Icon name="plus" size={16} color={colors.primary.contrastText} library="MaterialCommunityIcons" />
          </TouchableOpacity>
        </View>
      </View>
    </Card>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Icon name="shopping-bag" size={64} color={colors.icon.tertiary} library="Feather" />
