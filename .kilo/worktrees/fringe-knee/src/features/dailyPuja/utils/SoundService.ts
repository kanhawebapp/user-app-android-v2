...
const initAudio = (): void => {
  ...
  console.log('[SoundService] Audio initialized, using native:', useNativeModule);
};
...
const loadSound = (soundName: string): Promise<Sound> => {
  return new Promise((resolve, reject) => {
    ...
    const sound = new Sound(soundFile, undefined, error => {
      if (error) {
        console.error('[SoundService] Failed to load ' + soundFile + ':', error);
        reject(error);
        return;
      }
...
export const playEffect = async (soundName: string): Promise<void> => {
  try {
    console.log('[SoundService] playEffect: ' + soundName + ', native: ' + useNativeModule);
    ...
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        const didPlay = sound.play(success => {
          ...
          if (!success) {
            console.error('[SoundService] Playback failed: ' + soundName);
          }
        ...
        isPlaying[soundName] = true;
        console.log('[SoundService] Playing: ' + soundName + ', result: ' + didPlay);
        resolve();
      } catch (e) {
        console.error('[SoundService] Error playing ' + soundName + ':', e);
        reject(e);
      }
    });
  } catch (error) {
    console.error('[SoundService] playEffect error for ' + soundName + ':', error);
  }
};
...
export const playAarti = async (soundName: string = 'aarti'): Promise<void> => {
  try {
    console.log('[SoundService] playAarti: ' + soundName + ', native: ' + useNativeModule);
    ...
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        const didPlay = sound.play(success => {
          if (!success) {
            console.error('[SoundService] Aarti playback failed: ' + soundName);
          }
        });

        isPlaying[soundName] = true;
        soundObjects[soundName] = sound;
        console.log('[SoundService] Aarti playing: ' + soundName + ', result: ' + didPlay);
        resolve();
      } catch (e) {
        console.error('[SoundService] Error playing aarti ' + soundName + ':', e);
        reject(e);
      }
    });
  } catch (error) {
    console.error('[SoundService] playAarti error for ' + soundName + ':', error);
  }
};
...
export const stopAarti = async (soundName: string = 'aarti'): Promise<void> => {
  try {
    console.log('[SoundService] stopAarti: ' + soundName + ', native: ' + useNativeModule);
    ...
    console.error('[SoundService] Stop aarti error:', error);
  }
};
...
export const stopAllSounds = async (): Promise<void> => {
  try {
    ...
    console.error('[SoundService] Stop all error:', error);
  }
};
  ...
  try {
    ...
    if (useNativeModule) {
      for (const key of Object.keys(SOUND_FILES)) {
        try {
          ...
          console.error('[SoundService] Failed to preload (native): ' + key, error);
        }
      }
    ...
    for (const key of keys) {
      try {
        ...
        console.error('[SoundService] Failed to preload: ' + key, error);
      }
    }

    console.log('[SoundService] Preloading complete');
  } catch (error) {
    console.error('[SoundService] Preload error:', error);
  }
};
