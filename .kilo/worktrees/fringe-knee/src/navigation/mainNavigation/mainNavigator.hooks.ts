...
export interface UseSidebarNavigationReturn {
  ...
  handleNavigateToAstrologerProfile: (astrologerId: string) => void;
...
export const useSidebarNavigation = ({
  ...
  const previousSidebarRef = useRef<SidebarScreenKey | null>(null);
  ...
  const handleSidebarBack = useCallback(() => {
    if (sidebarScreen === 'astrologer_profile' && previousSidebarRef.current) {
      setSidebarScreen(previousSidebarRef.current);
  ...
  const handleNavigateToAstrologerProfile = useCallback((astrologerId: string) => {
    previousSidebarRef.current = sidebarScreen;
    setSelectedAstrologerId(astrologerId);
    setSidebarScreen('astrologer_profile');
  }, [sidebarScreen]);
...
export const useAuthUser = ({onLogout}: UseAuthUserProps = {}): UseAuthUserReturn => {
...
export const useNotificationCount = (): number => {
  // In production, this could be connected to actual notification store
