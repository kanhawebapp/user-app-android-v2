...
const AnimatedAppContent: React.FC<AnimatedAppContentProps> = ({
  ...
  const hasNavigatedToCallRef = React.useRef(false);
...
// Navigate to CallScreen when queue resolves to call state
   React.useEffect(() => {
     if (
       shouldNavigateToCall &&
       participant &&
   ...
   }, [shouldNavigateToCall, participant, callId]);

  // Reset navigation ref when returning to main flow
  React.useEffect(() => {
    if (
      callStatus === 'ended' ||
      callStatus === 'rejected' ||
