...
class WebRTCService {
  ...
  private _flushPendingAnswer(): void {
   ...
   emitState(
     ...
     this.onStateChange?.({
       ...
       participant: partial.participant !== undefined ? partial.participant : current.participant,
       status,
       localStream: this.localStream,
       remoteStream: this.remoteStream,
       isMuted,
       isSpeakerOn: this.isSpeakerOn,
       callDuration: partial.callDuration !== undefined ? partial.callDuration : current.callDuration,
       error: partial.error !== undefined ? partial.error : current.error,
       callDurationRemaining: partial.callDurationRemaining !== undefined ? partial.callDurationRemaining : current.callDurationRemaining,
     });
   }
    ...
    (pc as any).onicecandidate = (event: any) => {
     ...
     (pc as any).ontrack = (event: any) => {
       ...
       if (event.streams && event.streams[0]) {
        ...
        // Only emit connected if we are not already connected to avoid state resets
        const currentStoreStatus = useCallStore.getState().status;
        if (currentStoreStatus !== 'connected') {
          console.log(
            '[WebRTC] Emitting connected state (current status: %s)',
            currentStoreStatus,
          );
          const current = useCallStore.getState();
          this.emitState('connected', {
            ...
            callDuration: current.callDuration,
          });
        } else {
          console.log(
            '[WebRTC] Already in connected state, skipping emitState from ontrack',
          );
        }
     };
     ...
     (pc as any).onconnectionstatechange = () => {
       ...
       this._logRTPStats();
         ...
         if (state === 'connected') {
           ...
           console.log(
             '[WebRTC] connectionState=connected → safeSetStatus(connected) [store=%O]',
             useCallStore.getState(),
           );
           useCallStore.safeSetStatus('connected', 'onconnectionstatechange');
           console.log(
             '[WebRTC] after safeSetStatus - useCallStore.status:',
             useCallStore.getState().status,
           );
           console.log(
             '[WebRTC] calling emitState(connected) [store=%O]',
             useCallStore.getState(),
           );
           this.emitState('connected');
         }
       ...
       if (state === 'failed') {
         ...
         // IGNORE failure if we have remote stream or were recently connected
         ...
         if (shouldIgnoreFailure) {
           console.log(
             '[WebRTC] Connection failed but ignoring due to: remoteStreamReceived=%s, remoteStream=%s, currentStatus=%s, recentlyConnected=%s',
             this.remoteStreamReceived,
             !!this.remoteStream,
             currentStatus,
             recentlyConnected,
           );
           return; // Do NOT reconnect, cleanup, reset state, or restart offer
         }
         
         // Only attempt reconnect if we don't have a working connection
         console.log(
  ...
  async startCallAsCaller(request: CallRequestData): Promise<void> {
    try {
      ...
      await this._startAudioSession();
       ...
       useCallStore.safeSetStatus('calling', 'startCallAsCaller');
       useCallStore.setLocalStream(this.localStream);
    } catch (error: any) {
      this.cleanup();
  ...
  async acceptCall(request: CallRequestData): Promise<void> {
    try {
      ...
      socketService.emit('webrtc_call_accepted', {
       ...
       useCallStore.safeSetStatus('connecting', 'acceptCall');
       useCallStore.setLocalStream(this.localStream);
       useCallStore.setParticipant({
         id: request.callerId,
  ...
  private async _createAndSendOffer(): Promise<void> {
     ...
     useCallStore.safeSetStatus('creating_offer', '_createAndSendOffer');
     useCallStore.setLocalStream(this.localStream);
     ...
     useCallStore.safeSetStatus('waiting_answer', '_createAndSendOffer');
  }
  ...
  async handleOffer({
    ...
    try {
      ...
      useCallStore.safeSetStatus('waiting_connection', 'handleOffer');
    } catch (error: any) {
      this.emitState('ended', {
  ...
  cleanup(stopAudio: boolean = true) {
    ...
    console.log(
     ...
     useCallStore.safeSetStatus('ended', 'cleanup');
  }
