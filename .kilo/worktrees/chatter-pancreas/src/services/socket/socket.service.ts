...
class SocketService {
  ...
  private _signalingQueue: Array<{event: string; data: any}> = [];

  private readonly _signalingEvents: ReadonlySet<string> = new Set([
  ...
  private _isSignalingEvent(event: string): boolean {
    return this._signalingEvents.has(event);
  }

  private _bufferSignalingEvent(event: string, data: any): void {
    this._signalingQueue.push({event, data});
    console.log(
      `[SocketService] Buffered signaling event "${event}" — queue size:`,
      this._signalingQueue.length,
    );
  }

  flushSignalingQueue(): void {
    if (this._signalingQueue.length === 0) return;
    if (!this.socket || !this.socket.connected) return;

    const queue = [...this._signalingQueue];
    this._signalingQueue = [];

    for (const {event, data} of queue) {
      try {
        (this.socket as any).emit(event, data);
        console.log(`[SocketService] Flushed queued event:`, event);
      } catch (err) {
        console.error(`[SocketService] Failed to flush event`, event, err);
      }
    }
  }
  ...
  onReconnect(): void {
    console.log(
      '[SocketService] Socket reconnected — flushing',
      this._signalingQueue.length,
      'queued signaling events',
    );
  ...
  private handleChatAccepted = (
    ...
    // Transition call store from 'calling' → 'connecting'
    // (idempotent: chatAcceptedHandled guard prevents re-entry)
  ...
  private registerGlobalListeners(
    ...
    (socket as any).on(SOCKET_EVENTS.CHAT_ACCEPTED, (data: RawChatAcceptedData) => {
  ...
  emit(event: string, data: any): boolean {
    ...
    if (!this.socket.connected) {
      if (this._isSignalingEvent(event)) {
        this._bufferSignalingEvent(event, data);
        return true;
      }
