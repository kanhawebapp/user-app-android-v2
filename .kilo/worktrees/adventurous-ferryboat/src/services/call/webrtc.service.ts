...
class WebRTCService {
  ...
  private _flushPendingAnswer(): void {
    ...
    console.log(
      '[WebRTC] Flushing queued answer — room_id:',
      room_id,
    );
  ...
  private _createPeerConnection(): RTCPeerConnection {
    ...
    (pc as any).ontrack = (event: any) => {
      ...
      if (event.streams && event.streams[0]) {
        ...
        if (!this.hasEmittedConnected) {
          this.hasEmittedConnected = true;
          console.log('[WebRTC] ontrack (no stream) emitting connected (first time)');
          this.emitState('connected');
        } else {
          console.log('[WebRTC] ontrack (no stream) skipping connected (already emitted)');
        }
      }
    };
  ...
  async handlePeerJoined(request: CallRequestData): Promise<void> {
    try {
      ...
      if (this.peerJoinedReceived) {
        console.log('[WebRTC] peer_joined already received — skipping duplicate');
        return;
      }
