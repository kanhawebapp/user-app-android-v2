...
export const useCall = () => {
    ...
    // Initialize the call timer hook
    const { startCountdown, stopCountdown, resetCountdown, isRunning } = useCallTimer();
  ...
  const navigation = useNavigation();
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        // Note: Timer cleanup is handled by useCallTimer hook
      };
    }, [resetCallTimer]);
   ...
   // ── Countdown ──────────────────────────────────────────────────────────
    ...
    useEffect(() => {
      ...
      // Start the countdown timer with the current remaining time
      startCountdown(callDurationRemaining);

      return () => {
        // Cleanup is handled by the useCallTimer hook's useEffect
      };
