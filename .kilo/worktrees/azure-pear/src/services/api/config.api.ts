...
export const fetchAppConfig = async (): Promise<AppConfiguration> => {
  try {
    ...
    throw new Error(
      response.data?.error?.message || 'Failed to fetch configuration',
    );
  } catch (error) {
...
export const fetchBrandingConfig = async () => {
  try {
    const response = await api.get<ConfigApiResponse>(
      CONFIG_ENDPOINTS.BRANDING,
    );
    return response.data?.data;
  } catch (error) {
    loggingService.error('[ConfigAPI] Failed to fetch branding config', {
      error,
    });
    throw error;
  }
