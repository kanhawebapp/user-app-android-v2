...
const RemediesScreen: React.FC<RemediesScreenProps> = ({
  ...
  const filteredRemedies = remedies.filter(remedy => {
    ...
    const matchesCategory =
      selectedCategory === 1 ||
      remedy.category === remedyCategories[selectedCategory - 1].name;
    return matchesSearch && matchesCategory;
  });

  const getCategoryIcon = (icon: string) => {
    return icon === 'temple-buddhist' ||
      icon === 'charity' ||
      icon === 'music-note' ||
      icon === 'diamond-stone'
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'Puja':
        return {
          bg: colors.common.purple[50] || '#f3e5f5',
          text: colors.common.purple[500] || '#9c27b0',
        };
      case 'Donation':
        return {
          bg: colors.common.green[50] || '#e8f5e9',
          text: colors.common.green[500] || '#4caf50',
        };
      case 'Mantra':
        return {
          bg: colors.common.orange[50] || '#fff3e0',
          text: colors.common.orange[500] || '#ff9800',
        };
      case 'Gemstone':
        return {
          bg: colors.common.blue[50] || '#e3f2fd',
          text: colors.common.blue[500] || '#2196f3',
        };
      default:
        return {bg: colors.primary.light + '20', text: colors.primary.main};
    }
  };
  ...
  const renderRemedyCard = (remedy: (typeof remedies)[0]) => {
    ...
    return (
      <Card key={remedy.id} style={styles.remedyCard}>
        ...
        <Text variant="subtitle" weight="semibold" style={{marginTop: 8}}>
          {remedy.name}
        </Text>
        <Text
          variant="bodySmall"
