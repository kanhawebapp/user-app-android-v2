...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  const banners = useMemo(() => {
    /**
     * If parent data available then use it
     */
    if (data?.length > 0) {
      return data;
    }
    ...
    if (bannerResponse?.length > 0) {
      return bannerResponse.map((item: any, index: number) => ({
        id: item?.id || `${index}`,
        title: item?.heading || '',
     ...
     */
    return DEFAULT_BANNER_IMAGES.map((image, index) => ({
      id: `${index + 1}`,
      imageUrl: image,
      title: '',
      subtitle: '',
  ...
  const infiniteBanners = useMemo(() => {
    ...
    return [
      {...last, id: `clone-last-${last.id}`},
      ...banners,
      {...first, id: `clone-first-${first.id}`},
    ];
  }, [banners]);

  const [activeIndex, setActiveIndex] = useState(
    infiniteBanners.length > 1 ? 1 : 0,
  );
    ...
    <View style={[styles.container, style]}>
      <ScrollView
        ...
        contentContainerStyle={{
          width: ITEM_WIDTH * infiniteBanners.length,
        }}
        onMomentumScrollEnd={handleMomentumScrollEnd}
        onScrollBeginDrag={handleScrollBeginDrag}
        onScrollEndDrag={handleScrollEndDrag}>
        {infiniteBanners.map((item, index) => (
          <View
            ...
            <TouchableOpacity
              activeOpacity={0.92}
              style={styles.slideTouchable}
              onPress={() => handleBannerPress(item?.ctaAction)}>
              <ImageBackground
                source={
