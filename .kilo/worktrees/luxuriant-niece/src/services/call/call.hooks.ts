  ...
  const startCall = useCallback(
    async (data: CallRequestData) => {
      try {
        ...
        // CRITICAL: Register listeners BEFORE emitting join_call
        console.log('[USER CALL] Registering peer_joined listener');
        console.log('[USER CALL] Registering answer listener');
        console.log('[USER CALL] Registering ice-candidate listener');

        (socket as any).off('peer_joined');
        (socket as any).off('answer');
        (socket as any).off('ice-candidate');

        (socket as any).on('peer_joined', (eventData: any) => {
          console.log('[USER CALL] peer_joined event fired', eventData);
          const callData: CallRequestData = {
            callId: webRTCService.getCallId() || data.callId,
            callerId: webRTCService.getCallerId() || data.callerId,
            calleeId: webRTCService.getCalleeId() || data.calleeId,
            roomId: webRTCService.getRoomId() || data.roomId,
        ...
        (socket as any).on('answer', (eventData: any) => {
          console.log('[USER CALL] answer event received', eventData);
          webRTCService.handleAnswer({
            sdp: eventData.answer,
            room_id: eventData.room_id,
          });
        });

        (socket as any).on('ice-candidate', (eventData: any) => {
          console.log('[USER CALL] ice-candidate event received', eventData);
        ...
        // NOW emit join_call
        console.log('[USER CALL] About to emit join_call');
        socket.emit('join_call', {
          room_id: data.roomId,
          user_id: data.callerId,
        });
        console.log('[USER CALL] join_call emitted successfully');
