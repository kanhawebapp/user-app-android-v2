...
const loadSound = (soundName: string): Promise<Sound> => {
  return new Promise((resolve, reject) => {
    ...
    const sound = new Sound(soundFile, basePath, error => {
      if (error) {
        console.log(`[SoundManager] Failed to load: ${soundFile}`, error);
        reject(error);
        return;
      }
  ...
  return new Promise(resolve => {
    try {
      ...
      const didPlay = sound.play(success => {
        if (!success) {
          console.log(`[SoundManager] Playback failed: ${soundName}`);
        } else {
      ...
      console.log(`[SoundManager] Error playing sound: ${soundName}`, e);
      resolve(false);
    }
  });
};
...
export const playAarti = async (soundName: string = 'aarti'): Promise<void> => {
  try {
    ...
    console.log('[SoundManager] playAarti error:', error);
  }
};
...
export const stopAarti = async (soundName: string = 'aarti'): Promise<void> => {
  try {
    ...
    console.log('[SoundManager] Stop aarti error:', error);
  }
};
...
export const playEffect = async (soundName: string): Promise<void> => {
  try {
    ...
    console.log(`[SoundManager] playEffect error for ${soundName}:`, error);
  }
};
...
export const stopAllSounds = async (): Promise<void> => {
  try {
    ...
    console.log('[SoundManager] Stop all sounds error:', error);
  }
};
  ...
  try {
    ...
    for (const key of soundKeys) {
      try {
        ...
        console.log(`[SoundManager] Failed to preload: ${key}`, error);
      }
    }

    console.log('[SoundManager] Sounds preloading complete');
  } catch (error) {
    console.log('[SoundManager] Preload sounds error:', error);
  }
};
