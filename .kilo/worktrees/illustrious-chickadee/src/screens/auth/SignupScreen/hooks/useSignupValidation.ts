...
export const useSignupValidation = ({
  ...
  const validateConfirmPassword = useCallback((value: string, pwd: string): string | null => {
    if (!value || value.length === 0) {
  ...
  const isValid = useMemo(
    ...
    [isNameValid, isEmailValid, isPhoneValid, isPasswordValid, isConfirmPasswordValid]
  );

  // Get field error
  const getFieldError = useCallback((field: keyof SignupFormData): string | null => {
    switch (field) {
      case 'name':
        return nameError;
      case 'email':
  ...
  const validateAll = useCallback((): boolean => {
    ...
    const newConfirmPasswordError = validateConfirmPassword(confirmPassword, password);
  ...
  }, [name, email, phone, password, confirmPassword, validateName, validateEmail, validatePhone, validatePassword, validateConfirmPassword]);
