...
const ChatCallScreen: React.FC<ChatCallScreenProps> = ({
  ...
  const [chatUserData, setChatUserData] = useState<ChatRequestData | null>(
    null,
  );
  ...
  const handleChatRequestSubmit = useCallback(
    async (data: ChatRequestData) => {
      ...
      // Callback for external navigation if needed
      onNavigateToChat?.(selectedAstrologer, data);
    },
    [selectedAstrologer, onNavigateToChat],
  );
  ...
  if (showChatScreen && selectedAstrologer && chatUserData) {
    return (
      <ChatScreen
        astrologerName={selectedAstrologer.name}
        astrologerImage={selectedAstrologer.image}
        astrologerRating={selectedAstrologer.rating}
        astrologerExperience={selectedAstrologer.experience}
        astrologerSkills={selectedAstrologer.skills}
        isOnline={selectedAstrologer.isAvailableForChat}
