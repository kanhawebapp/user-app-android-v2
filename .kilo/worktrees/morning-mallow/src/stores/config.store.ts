  ...
  persist(
    (set, get) => ({
      ...
      fetchConfig: async () => {
        ...
        if (isLoading) {
          loggingService.info(
            '[ConfigStore] Fetch already in progress, skipping...',
          );
          return;
        }
        ...
        try {
          ...
          loggingService.info(
            '[ConfigStore] Configuration updated successfully',
            {
...
// Select API config
export const useApiConfig = () => useConfigStore(state => state.config?.api);

// Select texts
export const useAppTexts = () => useConfigStore(state => state.config?.texts);
...
// Select limits
export const useAppLimits = () => useConfigStore(state => state.config?.limits);
...
export const useAppName = () =>
  useConfigStore(
    state =>
      state.config?.branding?.appName || DEFAULT_APP_CONFIG.branding.appName,
  );

export const useAppTagline = () =>
  useConfigStore(
    state =>
      state.config?.branding?.tagline || DEFAULT_APP_CONFIG.branding.tagline,
  );
...
export const useCompanyName = () =>
  useConfigStore(
    state =>
      ...
      DEFAULT_APP_CONFIG.branding.companyName,
  );
...
export const usePrimaryColor = () =>
  useConfigStore(
    state =>
      ...
      DEFAULT_APP_CONFIG.branding.primaryColor,
  );

export const useSecondaryColor = () =>
  useConfigStore(
    state =>
      state.config?.branding?.secondaryColor ||
      DEFAULT_APP_CONFIG.branding.secondaryColor,
  );

export const useApiBaseUrl = () =>
  useConfigStore(
    state => state.config?.api?.baseUrl || DEFAULT_APP_CONFIG.api.baseUrl,
  );
...
export const useFeatureEnabled = (
  feature: keyof typeof DEFAULT_APP_CONFIG.features,
) =>
  useConfigStore(
    state =>
      state.config?.features?.[feature] ?? DEFAULT_APP_CONFIG.features[feature],
  );
...
export const useOnboardingTagline = () =>
  useConfigStore(
    state =>
      state.config?.texts?.onboardingTagline ||
      DEFAULT_APP_CONFIG.texts.onboardingTagline,
  );

export const useOnboardingHeadline = () =>
  useConfigStore(
    state =>
      state.config?.texts?.onboardingHeadline ||
      DEFAULT_APP_CONFIG.texts.onboardingHeadline,
  );

export const useOnboardingBenefits = () =>
  useConfigStore(
    state =>
      state.config?.texts?.onboardingBenefits ||
      DEFAULT_APP_CONFIG.texts.onboardingBenefits,
  );
...
export const useSplashAutoNavigateTimeout = () =>
  useConfigStore(
    state =>
      state.config?.texts?.splashAutoNavigateTimeout ||
      DEFAULT_APP_CONFIG.texts.splashAutoNavigateTimeout,
  );
...
export const useWebRtcConfig = () => {
  ...
  return {
    stunServer:
      config?.api?.webRtcStunServer || DEFAULT_APP_CONFIG.api.webRtcStunServer,
    turnServer:
      config?.api?.webRtcTurnServer || DEFAULT_APP_CONFIG.api.webRtcTurnServer,
    turnUsername:
      config?.api?.webRtcTurnUsername ||
      DEFAULT_APP_CONFIG.api.webRtcTurnUsername,
    turnPassword:
      config?.api?.webRtcTurnPassword ||
      DEFAULT_APP_CONFIG.api.webRtcTurnPassword,
    signalingServerUrl:
      config?.api?.signalingServerUrl ||
      DEFAULT_APP_CONFIG.api.signalingServerUrl,
  };
};
...
export const useEnvironment = () =>
  useConfigStore(
    state =>
      ...
      DEFAULT_APP_CONFIG.api.environment ||
      'development',
  );
