...
export const HeroBanner: React.FC<HeroBannerProps> = ({
  ...
  // Auto-scroll effect
  useEffect(() => {
    if (infiniteBanners.length <= 1 || !isAutoScrolling) return;
  ...
  const renderItem = ({item}: {item: HeroBannerData & {isClone?: boolean}}) => (
    <View style={styles.slideContainer}>
      <TouchableOpacity
        activeOpacity={0.9}
        onPress={() => !item.isClone && onCtaPress?.(item.ctaAction)}
