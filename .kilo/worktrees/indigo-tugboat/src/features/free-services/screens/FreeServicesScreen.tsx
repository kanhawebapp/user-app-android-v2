...
const FreeServicesScreen: React.FC<FreeServicesScreenProps> = ({ onNavigateBack }) => {
  ...
  const freeServices = [
    {
      ...
      description: 'Get your daily horoscope predictions based on your zodiac sign',
      icon: 'stars',
    ...
    },
    {
      id: '3',
      title: 'Match Making',
      description: 'Check compatibility with your partner using Kundli matching',
      icon: 'favorite',
      ...
      >
        ...
        <Card style={styles.bannerCard}>
          <View style={styles.bannerContent}>
            <Icon name="card-giftcard" size={40} color={colors.common.white} library="MaterialIcons" />
            <View style={styles.bannerText}>
              <Text variant="h6" weight="bold" style={{ color: colors.common.white }}>
                Free Astrology Services
              </Text>
              <Text variant="bodySmall" style={{ color: colors.common.white, opacity: 0.9, marginTop: 4 }}>
          ...
          {freeServices.map((service) => (
            <TouchableOpacity key={service.id} style={styles.serviceCard} activeOpacity={0.7}>
              <View style={[styles.serviceIcon, { backgroundColor: service.backgroundColor }]}>
                <Icon name={service.icon} size={28} color={service.iconColor} library="MaterialIcons" />
              </View>
          ...
          <Card style={styles.articlesCard}>
            {popularArticles.map((article, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.articleItem,
                  index < popularArticles.length - 1 && { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border.light }
                ]}
                activeOpacity={0.7}
              >
                <Icon name="article" size={20} color={colors.icon.secondary} library="MaterialIcons" />
                <View style={styles.articleInfo}>
                  <Text variant="body" style={{ color: colors.text.primary }}>{article.title}</Text>
                  <Text variant="captionSmall" style={{ color: colors.text.tertiary, marginTop: 2 }}>
                    {article.views} views
                  </Text>
                </View>
                <Icon name="chevron-right" size={20} color={colors.icon.tertiary} library="MaterialIcons" />
              </TouchableOpacity>
            ))}
          </Card>
        </View>
        ...
        <Card style={styles.ctaCard}>
          <View style={styles.ctaContent}>
            <Icon name="workspace-premium" size={32} color={colors.primary.main} library="MaterialIcons" />
            <View style={styles.ctaText}>
              <Text variant="body" weight="semibold" style={{ color: colors.text.primary }}>
                Unlock Premium Services
              </Text>
              <Text variant="captionSmall" style={{ color: colors.text.secondary, marginTop: 4 }}>
