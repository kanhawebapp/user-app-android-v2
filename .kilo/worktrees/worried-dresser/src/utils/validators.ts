...
export const validatePhoneNumber = (
  ...
  if (!cleanedPhone) {
    ...
    errors.push(`Phone number must be less than ${VALIDATION_RULES.PHONE_MAX_LENGTH} digits`);
...
export const validateName = (
  ...
  if (!trimmed) {
    ...
    errors.push(`${fieldName} must be less than ${VALIDATION_RULES.NAME_MAX_LENGTH} characters`);
...
export const validateMessage = (content: string): ValidationResult => {
  ...
  if (!trimmed) {
    ...
    errors.push(`Message must be less than ${VALIDATION_RULES.MESSAGE_MAX_LENGTH} characters`);
  }
